		...
		},
		{
			...
			buildInputFn: func(i *input) {
				handler := librest.GatewayHandlerFunc(
					librest.ApplyGatewayMiddlewares(
						func(w http.ResponseWriter, r *http.Request) error {
							return liberr.NewBaseError(liberr.NewErrorDetails("Error Happened", entity.ErrorCodeOrderNotFound, "field"))
						}, middlewares...,
					),
				)
			...
			assertFn: func(i *input) {
				expected := &entity.ErrorResponse{
					Errors: []*entity.Error{
						{ErrorMessage: "Error Happened", ErrorCode: entity.ErrorCodeOrderNotFound, ErrorField: "field"},
					},
					Meta: &entity.Meta{
